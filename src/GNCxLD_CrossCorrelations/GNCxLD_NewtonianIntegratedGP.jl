...
between the Newtonian effect arising from the Galaxy Number Counts (GNC) and the Integrated 
...
This function is used inside `ξ_GNCxLD_Newtonian_IntegratedGP` with the trapz() from the 
...
It's computed through the symmetric function `ξ_GNCxLD_Newtonian_IntegratedGP`; check its documentation for
...
[`ξ_GNCxLD_Newtonian_IntegratedGP`](@ref)
"""
