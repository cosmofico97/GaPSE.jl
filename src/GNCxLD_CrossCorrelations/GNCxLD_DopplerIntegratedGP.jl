...
between the Doppler effect arising from the Galaxy Number Counts (GNC) and the Integrated 
...
This function is used inside `ξ_GNCxLD_Doppler_IntegratedGP` with the trapz() from the 
...
Doppler effect arising from the Galaxy Number Counts (GNC) and the Integrated 
Gravitational Potential (GP) one arising from the Luminosity Distance (LD) perturbations.
...
```math
\\begin{split}
    \\xi^{v_{\\parallel} \\int\\!\\phi} (s_1, s_2, y) =
    D_1 \\int_0^{s_2} \\! \\mathrm{d}\\chi_2 \\;
    \\mathfrak{J}^{v_{\\parallel} \\int\\!\\phi}_{31} \\,I_1^3(\\Delta\\chi_2) \\, ,
\\end{split}
```
...
function ξ_GNCxLD_Doppler_IntegratedGP(s1, s2, y, cosmo::Cosmology;
    ...
    int_ξs = [
        en * GaPSE.integrand_ξ_GNCxLD_Doppler_IntegratedGP(IP, P1, P2, y, cosmo; kwargs...)
        for IP in IPs
    ]
...
It's computed through the symmetric function `ξ_GNCxLD_Doppler_IntegratedGP`; check its documentation for
...
[`ξ_GNCxLD_Doppler_IntegratedGP`](@ref)
"""
function ξ_LDxGNC_IntegratedGP_Doppler(s1, s2, y, cosmo::Cosmology; 
    b1=nothing, b2=nothing, s_b1=nothing, s_b2=nothing,
    ...
    ξ_GNCxLD_Doppler_IntegratedGP(s2, s1, y, cosmo; 
